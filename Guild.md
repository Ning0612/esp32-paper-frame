...
因此圖片格式必須包含 `palette\_version`，MVP 先採官方 driver 實際支援的六個有效色彩，避免 WebUI 預覽與實機顯示不一致。
...
format\_version    1 byte

palette\_version   1 byte
...
&#x20;   virtual bool readRaw(uint16\_t\& output) = 0;
...
\* 發送 `presence\_return`／`presence\_away` command。
