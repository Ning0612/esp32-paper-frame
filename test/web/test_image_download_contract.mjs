...
assert.ok(server.includes("image_reorder_handler"));
assert.ok(server.includes("start_image_mutation_task"));
assert.ok(server.includes("worker->activate_image"));
assert.ok(server.includes("worker->remove_image"));
assert.ok(server.includes("worker->reorder_images"));
assert.ok(server.includes('method = HTTP_DELETE'));
assert.ok(server.includes('method = HTTP_PUT'));
assert.ok(server.includes("receive_image_order_body"));
assert.ok(!server.includes('httpd_resp_set_hdr(request, "Content-Length"'));
assert.ok(serializer.includes("attachment; filename=\\\""));
console.log("image_download_contract: 39 tests passed");
