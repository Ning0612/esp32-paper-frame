...
void test_form_accepts_masked_key_update_fields()
{
    const char body[] =
        ...
        "units=metric&ntp_server=pool.ntp.org&api_key=newkey123";
...
{
    const char body[] =
        "latitude_e6=0&longitude_e6=0&"
        "units=imperial&ntp_server=time.example";
    pf_web::WeatherConfigForm form{};
    TEST_ASSERT_EQUAL(
        pf_web::WeatherConfigParseStatus::ok,
        pf_web::parse_weather_config_form(body, std::strlen(body), form));
...
void test_form_rejects_unknown_duplicate_and_bad_encoding()
{
    const char common[] =
        "latitude_e6=0&longitude_e6=0&"
        "units=metric&ntp_server=time.example";
    pf_web::WeatherConfigForm form{};
    char duplicate[256]{};
