...
- 停用是 `SensorTask` 取樣迴圈的第一個分支，該通道的 ADC **完全不會被讀**，
  ...
  其他 decision-capable 通道時跳過已停用／未偵測到／讀取錯誤的通道）。
- 啟用但沒接線時**沒有防線**：`raw ≤ 10` 或 `raw ≥ 4085` 只回報
  `low_clipped`／`high_clipped`，仍然照常餵入判定
  （[ADR-0020](../adr/0020-light-clip-is-diagnostic-not-presence-gate.md)——
  clip 只是診斷資訊，不再讓通道退出判定）。**未接線的 ADC 腳也不保證落在
  這個區間**——ESP32-S3 的 ADC 輸入沒有內部上下拉，分壓電路不在時該腳是
  高阻抗，實際讀到的是飄動的中間值，韌體無法與真實讀值區分，一樣會被當成
  `online` 參與判定。
- 失效模式是不對稱的。合併規則是 AND（兩顆都低於各自 threshold 才算暗），
  所以浮接值（不論落在中間值還是剛好貼到某一端）若解讀成高於 threshold，
  那顆會永遠看起來「有光」，**裝置將永遠不會進入離席白屏，而且不會有任何
  錯誤訊息**。解讀成低於 threshold 反而無害，因為真正接著的那顆說了算。
...
**上下順序不可反接**：韌體判定的極性是「亮→讀值高、暗→讀值低」——`online`
狀態下具體比較式是 `filtered_raw < threshold → 暗`，貼著兩端極限時
（`low_clipped`／`high_clipped`）改由狀態直接決定方向而不比較 threshold
（見「光敏電阻分壓接法」一節下方與 [ADR-0020](../adr/0020-light-clip-is-diagnostic-not-presence-gate.md)），
但極性本身仍然一致。LDR 在上、固定電阻在下正好符合這個極性；反接會使
判定語意顛倒。
...
韌體把 raw ≤ 10 或 ≥ 4085 判為 `low_clipped`／`high_clipped`；
[ADR-0020](../adr/0020-light-clip-is-diagnostic-not-presence-gate.md) 之後
這兩個狀態仍會正常參與離席判定（clip 只是回報「貼到 ADC 極限」的診斷資訊），
但貼著極限時已經失去解析度——ADC 再往那個方向的光照變化都讀不出差異，讀值
本身也更容易受雜訊影響在極限附近抖動。所以你在意的整個光照範圍最好還是落在
raw 11–4084 之間；實際校正時若讀值頂到界線就換值：
...
3. **兩端餘裕原則上同等重要，暗端只是歷史上曾經代價更高**：在
   [ADR-0020](../adr/0020-light-clip-is-diagnostic-not-presence-gate.md)
   之前，全暗端貼著飽和門檻曾經會讓 presence 塌回 `unknown`、清空累積中的
   離席倒數（`update_presence()` 舊行為）；ADR-0020 之後 `low_clipped`／
   `high_clipped` 都正常參與判定，這個風險不對稱已經在判定層修掉。留在
   這裡的理由改為單純的解析度取捨：兩端都貼著極限時該方向的光照變化都讀
   不出差異，沒有哪一端天生更該犧牲，起始值算出來後照公式取幾何平均即可，
   不必再刻意往暗端偏。
...
兩端沒辦法同時留最大餘裕時，本專案曾經選擇犧牲亮端、保住暗端，原因（截至
2026-08-27，ADR-0020 之前）：

- **AND 語意本身就是為了讓誤判離席變難**（ADR-0018：「誤判離席（使用者
  在場卻白屏）比漏判離席（該睡沒睡）代價高得多」）。當時暗端貼著飽和
  門檻會讓 presence 塌回 `unknown`，等於用電路設計把 ADR 已經做的取捨又
  賠回去。
- 亮端飽和的實際影響很小：室內光與手遮這兩個「裝置多數時間會遇到」的
  條件都還在飽和門檻以內（見上表），只有「直射光」這種罕見邊界情境才會
  兩顆一起飽和，而且那種情境本來就已經確定是「亮」。

[ADR-0020](../adr/0020-light-clip-is-diagnostic-not-presence-gate.md)
（2026-08-29）之後這個偏好**不再是必要條件**：`low_clipped` 不會再讓
presence 塌回 `unknown`，兩顆同時貼著暗端一樣正確判斷為「暗」。現行
100 kΩ 保留下來是因為它同時也提供不錯的亮端餘裕（見上表），不是因為暗端
還有特殊風險——之後若要重新調整 `R_fix`，兩端可以用相同權重考慮，不必再
刻意偏暗。

判定規則：**每一顆已啟用且 decision-capable（`online`／`low_clipped`／
`high_clipped`）的通道都判為暗才算暗**——等價於「任一顆看到光就喚醒」
（ADR-0018）。`online` 通道用 raw 與自己的 threshold 比較；`low_clipped`
一律判為暗、`high_clipped` 一律判為亮，不經過 threshold 比較（threshold
是 0–4095 的可調欄位，直接拿去跟貼著極限的 raw 比較不保證方向正確，見
ADR-0020）。只有未啟用、未偵測到或讀取錯誤的通道會被忽略，不影響另一顆
（ADR-0020）。
...
- 光敏分壓的 `R_fix` 已於 2026-08-27 由 47 kΩ 換成 **100 kΩ**：47 kΩ 在
  ...
  而不是直接假設是判定邏輯的問題。**這次事件的根因已於 2026-08-29 在判定層
  修掉**（[ADR-0020](../adr/0020-light-clip-is-diagnostic-not-presence-gate.md)）：
  `low_clipped`／`high_clipped` 不再讓 presence 塌回 `unknown`，兩顆同時貼著
  暗端會正確判斷為「暗」而非清空離席倒數。上面這段歷史記錄與 `R_fix` 數值
  本身仍然有效——貼著極限依舊代表解析度變差，仍值得校正避免——但「貼著暗端
  會讓裝置卡在持續刷新」這個特定症狀不會再重演。
