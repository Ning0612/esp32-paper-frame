...
`R_fix` 目前採用 **47 kΩ**（兩通道皆同值；2026-08-24 由 10 kΩ 換上，取代
`R_fix ≈ √(R_亮 × R_暗)` 算出的幾何平均值 117 kΩ，改取兩端都留餘裕的近似值）。

2026-08-23 曾以 10 kΩ 驗證，足以分辨開燈與關燈，但全暗環境會貼到 ADC
底部（實測 raw ≤23，其中一通道直接 `saturated`），presence 因此在深暗
環境有塌成 `unknown`、離席白屏被取消的風險。2026-08-24 換成 47 kΩ 並重新
校正 threshold（兩通道皆設 2000）後實測（詳見
[VALIDATION.md](VALIDATION.md) 對應段落）：
...
| 手遮 | 2672 | 2385 |
| 全暗 | 777 | 781 |
| 直射光 | `saturated`（≥4085） | 3348 |

全暗讀值離低端飽和門檻（≤10）已有 750+ counts 餘裕，不會再誤觸飽和保護。
...
- 光敏分壓的 `R_fix` 已於 2026-08-24 由 10 kΩ 換成 **47 kΩ**：10 kΩ 在
  全黑環境會接近 ADC 底部而失去解析度（甚至觸發低端飽和保護），47 kΩ 實測
  全暗讀值落在 777–781，兩端都有充足 margin，見上方「光敏電阻分壓接法」
  一節的實測表。
