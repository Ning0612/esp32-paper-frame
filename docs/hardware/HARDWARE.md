...
| 光感測器 | 光敏電阻（LDR）＋分壓電路 **×2** | 通道 1 輸出接 `ADC1_CH4`、通道 2 接 `ADC1_CH6` | **可選**。兩顆同時接線已於 2026-08-23 完成實機驗證；**只接一顆的降級行為仍只有 host test**。`R_fix` 目前基線 10 kΩ |
...
2026-08-23 已用實機 ADC 讀值確認 10 kΩ 足以分辨開燈與關燈，但**全暗環境會
貼到 ADC 底部**——實測數值與該風險見下方「已知限制」的對照表，不要用這裡的
敘述代替它。只要判定「有沒有開燈」就夠用，需要分辨更暗的層次時改用 47 kΩ。韌體把 raw ≤ 10 或
...
- 光敏分壓的 `R_fix` = 10 kΩ 已用實機 ADC 讀值確認可分辨開燈／關燈，但在
  **全黑環境會接近 ADC 底部而失去解析度**；只要判定「有沒有開燈」就夠用，
  需要分辨更暗的層次時改用 47 kΩ。
- 供電方案尚未固定。外殼已有可列印的 CAD，但尚未定義列印材料與參數。
