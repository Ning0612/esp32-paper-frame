...
**後續更新（2026-08-24）**：使用者改變決定，換成 47 kΩ 並重新校正門檻，此風險已
解決——見下方「2026-08-24 — R_fix 換 47 kΩ 解決全暗飽和」段落的實測數據。
...
至此 Phase 7 感測器的**行為**全部實機閉環：DHT22 讀值與兩條拔除降級路徑、雙通道
...
## 2026-08-24 — R_fix 換 47 kΩ 解決全暗飽和

### 背景

2026-08-23 記錄的「已知限制：`R_fix` = 10 kΩ 在全暗時會飽和」在今天觸發了實際
影響：全暗時兩通道貼著低端飽和門檻（≤10），一旦兩通道同時 `saturated`，
presence 塌成 `unknown`，離席白屏被取消。使用者決定換成該段落已算出的
47 kΩ，兩顆通道的 `R_fix` 皆更換，並在 WebUI「環境與在場」頁重新校正
threshold（兩顆皆設為 2000，取代先前的 1200／1200）。

### 四種條件實測（GET /api/v1/sensors，登入後直接讀 API）

| 條件 | GPIO5 raw | GPIO7 raw | threshold |
| --- | --- | --- | --- |
| 室內光 | 3894 | 3178 | 2000 |
| 手遮 | 2672 | 2385 | 2000 |
| 全暗 | 777 | 781 | 2000 |
| 直射光（手電筒） | `saturated`（≥4085） | 3348 | 2000 |

### 結論

- **低端飽和問題已解決**：全暗時 raw 停在 777–781，離低端門檻（≤10）還有
  750+ counts 餘裕，兩顆維持 `online`，不會再塌成 `unknown`。與 2026-08-23
...
本次只確認了 ADC raw 值與飽和狀態，**未重跑計時的 AWAY/PRESENT debounce
