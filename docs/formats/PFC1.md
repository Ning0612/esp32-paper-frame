...
- 目錄最多 64 筆，序列化後最多 `32 + 64 × 128 = 8,224` bytes。這是政策
  常數（`kCatalogMaxEntries`），不是 wire format 的位元寬度上限——`count`
  欄位本身是 2 bytes，撐得住遠大於 64，調整這個常數不需要 bump
  `version`。但這不代表調整永遠零成本：
  [ADR-0010](../adr/0010-revert-catalog-cap-raise-ram-constraint.md)
  記錄了一次實機驗證失敗（拉到 96／64 都會讓
  `components/pf_web/health_server.cpp` 的圖片上傳／mutation task
  「有請求才動態配置」的 internal RAM stack 配置失敗），
  [ADR-0012](../adr/0012-raise-catalog-cap-after-ram-reclaim.md) 記錄了
  在 Phase 1 RAM 重構回收 57.8KB 之後重新實機驗證：64 穩定、80 因觀測到
  的 mutation task 最低餘裕過薄而拒絕採用，因此定案在 64。`pio run` 的
  靜態百分比不涵蓋執行期動態配置需求，未來要再調整這個常數，必須先
  完成實機驗證，不能只看編譯期報告。
...
- id、filename 不可重複；entry 數不得超過 64。
